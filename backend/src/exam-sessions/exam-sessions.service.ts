...
export class ExamSessionsService {
  ...
  async startExam(userId: string, startExamDto: StartExamDto) {
    ...
    // 2. Kiem tra thoi gian mo de
    const now = new Date();
    if (exam.openTime && now < exam.openTime) {
      throw new BadRequestException(
        `${ERROR_MESSAGES.EXAM_NOT_OPEN}. Thoi gian mo: ${exam.openTime.toISOString()}`
      );
    }

    if (exam.closeTime && now > exam.closeTime) {
      throw new BadRequestException(
        `${ERROR_MESSAGES.EXAM_CLOSED}. Thoi gian dong: ${exam.closeTime.toISOString()}`
      );
    }

    // 3. Kiem tra so lan thi
